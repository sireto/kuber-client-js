...
type Script = { cborHex: string, description: string, type: Type, keyHash?: never }
    | { cborHex?: never, description?: never, type: "sig", keyHash: string }

type TxInput = { utxo: string | UTxO, script?: never, redeemer?: never }
...
type Anchor = { url: string, dataHash: string, hash?: never }
    | { url: string, dataHash?: never, hash: string }

type Credential = { keyHash: string, scriptHash?: never }
    | { keyHash?: never, scriptHash: string }
...
type UpdateCommittee = { add: Record<string, number>, remove: string[], quorum: { numerator: number | bigint, denominator: number | bigint } }

type ParameterUpdate = { prevGovAction: string, MaxBlockExUnits: { exUnitsMem: number | bigint, exUnitsSteps: number | bigint } }

type TxProposal = { deposit: number | bigint, refundAccount: RefundAccount, anchor: Anchor }
