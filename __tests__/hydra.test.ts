...
describe("KuberHydraApiProvider Operations", async() => {
  ...
  test.runIf(head.state === 'Idle')("should Initialize Head", async function() {
  ...
  test.runIf(head.state === 'Final')("should re-Initialize Head", async function() {
    const result = await hydra.initialize(true);
    expect(result).to.exist;
    console.log("Head initialization result",result)
    hydra.waitForHeadState("Initial",180000)
  ...
  it.runIf(head.state == "Initial")("should attempt to Commit UTxOs", async () => {
    if (nodeAddr) {
      const nodeUtxos = await hydra.l1Api.queryUTxOByAddress(nodeAddr);
      console.log("Node address",nodeAddr,"value=",nodeUtxos.map(x=>x.txOut.value).reduce((a,b)=>a.add(b),new Value(0n)))
    ...
    const commitResult = await hydra.commit({
      utxos:[
        `${txIn.txHash.toString('hex')}#${txIn.index}`
      ]
    });
  ...
  it.skipIf(['Closed',"Final","Initial"].includes(head.state))
  ("should Close Head", async () => {
    await hydra.close(true);
    await hydra.waitForHeadState('Closed',180000,true);
  });

  it.runIf(head.state == 'Closed')
  ("should Fanout Head", async () => {
  ...
  it.runIf(head.state === 'Closed')
  ("should attempt to Contest", async () => {
