...
import {
  CommonProtocolParameters,} from "libcardano-wallet/utils/types";
...
export class KuberApiProvider extends KuberProvider {
  ...
  async queryUTxOByAddress(address: string): Promise<UTxO[]> {
    ...
    const response = await get(
      this.axios,
      "KuberApiProvider.queryUTxOByAddress",
      request,
      this.retry
    );
  ...
  async queryUTxOByTxIn(txIn: string): Promise<UTxO[]> {
    ...
    const response = await get(
      this.axios,
      "KuberApiProvider.queryUTxOByTxIn",
      request,
      this.retry
    );
    return toUTxO(response);
  ...
  async queryProtocolParameters(): Promise<CommonProtocolParameters> {
    ...
    const response = await get(
      this.axios,
      "KuberApiProvider.queryProtocolParameters",
      request,
      this.retry
    );
    return response;
  ...
  async querySystemStart() {
    ...
    return await get(
      this.axios,
      "KuberApiProvider.querySystemStart",
      request,
      this.retry
    );
  }
  ...
  async queryChainTip() {
    ...
    return await get(
      this.axios,
      "KuberApiProvider.queryChainTip",
      request,
      this.retry
    );
  }
  ...
  async buildTx(txBuilder: any, submit: boolean = false) {
    ...
    return await post(
      this.axios,
      `KuberApiProvider.buildTx_&_submit=${submit}`,
      request,
      txBuilder,
      this.retry
    );
  ...
  async calculateMinFee(tx: string): Promise<BigInt> {
    return post(
      this.axios,
      "KuberApiProvider.calculateMinFee",
      "api/v1/tx/fee",
      tx,
      this.retry
    ).then((res) => {
